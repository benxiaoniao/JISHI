...
2. **测试命令**：`python -m pytest tests/`（当前 1888 项全绿）。
   改完代码必须跑全量回归，不许留红。
...
- 📌 **下一步（新阶段三方向）**：路线图 §3.6 定的是「语言能力 / IDE 可用性 /
  ...
  **M41 已完成**；下一步是 **M42：语言服务补能力**（`textDocument/formatting` /
  `rename` / `codeAction` / `signatureHelp` / `workspace/symbol` /
  `semanticTokens`，每条都要有协议级测试），随后 **M43 编辑器客户端 + `jishi dap`**。
