    ...
    fn call_builtin_method(&mut self, name: String, receiver: Val,
        ...
        match (receiver, name.as_str()) {
            ...
            (Val::Dict(d), "更新") => {
                ...
                match &args[0] {
                    Val::Dict(other) => {
                        let items: Vec<(Val, Val)> = other.borrow().clone();
            ...
            (Val::Dict(d), "弹出") => {
                need_args(&name, args, 1, 2)?;
                let mut cur = d.borrow_mut();
                if let Some(pos) = cur.iter().position(|(k, _)| *k == args[0]) {
                    Ok(cur.remove(pos).1)
                } else if args.len() == 2 {
                    Ok(args[1].clone())
                } else {
                    Err(err("键错误", format!("字典里没有键「{}」", display(&args[0]))))
                }
            }
...
fn dict_set(d: &mut Vec<(Val, Val)>, k: Val, v: Val) {
    if let Some(slot) = d.iter_mut().find(|(ek, _)| *ek == k) {
        slot.1 = v;
    } else {
        d.push((k, v));
    }
...
fn new_builtins() -> HashMap<String, Val> {
    ...
    m.insert("最大".to_string(), Val::Builtin("最大", b_max));    m.insert("最小".to_string(), Val::Builtin("最小", b_min));
...
fn map_kwargs(qualified: &str, args: &[Val], knames: &[String], kwvals: &[Val])
    ...
    let (required, params) = match stdlib_sigs::sigof(qualified) {
        Some(s) => s,
    ...
    Ok(slots[..n].iter().map(|v| v.clone().unwrap()).collect())
}
