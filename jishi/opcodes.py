...
TAG_NAMES = {
    ...
    T_FUNC: "FUNC", T_ITER: "ITER", T_STR: "STR",
}
...
class Op:
    ...
    # -- M48 性能 --
    #: 方法调用融合（M48 阶段三②）：栈 [obj, 实参...] → 结果，a=实参数
    #: b=方法名下标（names 表）。一次宿主回调做完「取方法 + 调用」，
    #: 省掉 GET_ATTR 那次 ctypes 编组（实测每次约 4µs）。
    #:
    #: ⚠️ **只发给 C VM**（见 `runtime.py` 的 `Executor.can_fuse_method_call`）。
    #: 其它执行器与别的语言宿主**根本看不到这条指令**：树遍历不走字节码、
    #: Python VM / Node / Rust 拿到的是未融合的 GET_ATTR + CALL。所以
    #: **不需要改字节码格式版本**（`cache.py` 的键已含基石版本，会自然失效）。
    #: 万一别的宿主真收到了：它本来就会对未知指令报错——
    #: 但那是「编译器按宿主能力挑指令」的前提被破坏，属于内部错误。
    METHOD_CALL = 49
...
OP_ARITY = {
    ...
    Op.LIST_APPEND: 0,
    Op.BUILD_SLICE: 1,
    Op.METHOD_CALL: 2,     # M48：a=实参数 b=方法名下标
}
...
class CmpOp:
    ...
    0–5 是 **C VM 认识的快路径**（见 cvm/include/opcodes.h 的 JS_CMP_*），
    6–9 是 M23.2 新增的语义比较：C 侧 ``compare_fast`` 只处理 0–5，
    其余一律回落宿主回调 ``host->compare``，因此新增它们**不需要改 C 代码**。
    若将来要在 C 里加速这几个码，必须同步 cvm/include/opcodes.h。
    """
    ...
    LE = 4
    GE = 5
    #: 成员测试（M23.2）：`x 在 列表` / `x 不在 列表`
    IN = 6
    NOT_IN = 7
    #: 同一性判断（M23.2）：`x 是 空` / `x 不是 空`
    IS = 8
    IS_NOT = 9
...
SYMBOL_TO_CMPOP = {
    ...
    "在": CmpOp.IN, "不在": CmpOp.NOT_IN,
    "是": CmpOp.IS, "不是": CmpOp.IS_NOT,
}


class ParamKind:
    """形参种类（M25）：决定 C 侧 `bind_args` 谁负责收集多余实参。

    生成到 `cvm/include/opcodes.h` 的 `JS_PARAM_*`。
    """
...
class Instr:
    ...
    line: int = 0
    col: int = 0
    #: 若这条指令是**某个语句的第一条指令**，这里是那个语句的行号，否则 0。
    #: 调试器（B5）靠它认出「语句边界」：栈机没有这种事件，但编译器知道。
    #: 放在指令上而不是另开一张表，是为了让 VM 主循环里的一次判断就够了
    #: （实测：+15% → 更低；挂调试器才有这个开销，不挂时只多一次属性读取）。
    #: 注意 `as_tuple()` 仍是 6 元组——C 侧与跨语言宿主的布局不受影响。
    stmt_line: int = 0
...
class Code:
    ...
    #: 每个参数分到哪个单元（freevars/cellvars 部分）；非单元参数为 -1。
    param_cell: list[int] = field(default_factory=list)
    #: 形参种类（M25）：ParamKind.NORMAL / VARARGS / VARKW，与 params 对齐
    param_kind: list[int] = field(default_factory=list)
    ...
    @property
    def stmt_marks(self) -> list[int]:
        """**语句起始表**：与 `instrs` 等长，第 i 项是「第 i 条指令对应的语句行号」。

        0 表示这条指令不是语句的第一条。这就是给 C 侧（与跨语言宿主）用的形态——
        Python VM 直接读 `Instr.stmt_line` 更快，但 C 侧的指令是扁平 int32 数组，
        再塞一个字段就要动指令宽度（6→7），而**跨语言字节码格式**是公开契约，
        为一个调试功能改它不划算。所以另发一张平行的 int32 表（M40）。

        长度不足的位置按 0 处理，所以旧产物/缺表也不会出错。
        """
        return [ins.stmt_line for ins in self.instrs]
