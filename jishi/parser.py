...
#: 文本插值脱糖用的**内部保留名**（M40）。见 `_parse_fstring`：
#: 用 `$` 前缀是因为词法器把 `$` 判成「无法识别的字符」，用户**写不出**这个名字，
#: 于是它永远不会被 `导入 文本` 之类的用户代码遮蔽。三个执行器（树遍历 /
#: Python VM / C VM）与两个宿主（Node / Rust）都注册了这个内建。
INTERP_TO_TEXT = "$文本"
...
class Parser:
    ...
    def _parse_fstring(self, tok: Token) -> A.Node:
        ...
        插值用的是 **`$文本`**（内部保留名）而不是内建 `文本`（M40）：后者是
        用户可见的名字，一句 `导入 文本`（本项目最常用的标准库名）就会把它遮蔽，
        于是**所有 f-string 当场报「模块「文本」不能直接调用」**。`$` 在词法阶段
        就是「无法识别的字符」，用户写不出这个名字，所以永远不会被遮蔽。
        """
        ...
        for kind, val in parts:
            if kind == "text":
                ...
                nodes.append(A.Call(
                    func=A.Name(id=INTERP_TO_TEXT, line=tok.line, col=tok.col),
    ...
    def _parse_comp_tail(self, *, kind: str, elt: Optional[A.Node],                         key: Optional[A.Node] = None,
        ...
        self._expect_kw("在")
        # 可迭代对象**不认条件表达式**：紧随其后的「如果」是过滤子句
        # （`[x 遍历 x 在 y 如果 条件]`），不能被迭代对象吃掉（M36）
        it = self._parse_expr(allow_ternary=False)
    ...
    def _parse_for(self) -> A.Node:
        ...
        多变量（解包遍历，M41）走**解析期脱糖**：编译器只认单变量循环，
        所以这里改写成「单个临时循环变量 + 循环体开头一句解包赋值」：
        ...
        好处与 M26.2 的脱糖同一思路：**三个执行器与两个宿主一行都不用改**，
        行为必然一致（`令 a, b = …` 早就支持解包了）。
        """
        start = self._next()
        first_tok = self._expect_name("循环变量名")
        names = [A.Name(id=first_tok.value, line=first_tok.line,
                        col=first_tok.col)]
        star_index: Optional[int] = None
        while self._at_op(","):
            self._next()
            if self._at_op("*"):               # 星号解包：遍历 头, *尾 在 …
                star_tok = self._next()
                if star_index is not None:
                    self._err(ParseUnexpectedError,
                              "解包遍历里只能有一个「*」", tok=star_tok)
                star_index = len(names)
            tok = self._expect_name("循环变量名")
            names.append(A.Name(id=tok.value, line=tok.line, col=tok.col))
        self._expect_kw("在")
        it = self._parse_expr()
        self._expect_colon("遍历")
        body = self._parse_block()
        if len(names) == 1:
            return A.For(target=names[0], iter=it, body=body,
                         line=start.line, col=start.col)
        # 多变量：脱糖（临时名带 `__`，用户写不出来，见 M40 的「内部名」纪律）
        tmp = A.Name(id=f"__遍历项_{start.line}_{start.col}__",
                     line=start.line, col=start.col)
        unpack = A.Assign(target=A.TargetList(names=names,
                                              star_index=star_index,
                                              line=start.line, col=start.col),
                          op="=", value=tmp, line=start.line, col=start.col)
        return A.For(target=tmp, iter=it, body=[unpack] + list(body),
                     line=start.line, col=start.col)
    ...
    def _parse_case(self) -> tuple:
        ...
        if not wildcard:
            # 值这一层**不认条件表达式**：紧随其后的「如果」是**守卫**
            # （`情形 90 如果 加分：`），不能被值吃掉（M36）。
            # 想写条件表达式当值就加括号：`情形 (甲 如果 c 否则 乙)：`。
            patterns.append(self._parse_expr(allow_ternary=False))
            while self._at_op(","):
                self._next()
                patterns.append(self._parse_expr(allow_ternary=False))
        guard = None
    ...
    def _parse_expr(self, min_prec: int = 0,
                    allow_ternary: bool = True) -> A.Node:
        ...
        while True:
            ...
            if op is not None:
                ...
                continue
            break
        # M36 条件表达式 `甲 如果 条件 否则 乙`：优先级比「与/或」低（与 Python 的
        # conditional_expression 同级），所以只在本层最外面（min_prec == 0）识别。
        #
        # `allow_ternary=False` 专给**推导式的可迭代对象**用：
        # `[x 遍历 x 在 y 如果 条件]` 里的「如果」是**过滤子句**，
        # 迭代对象不能把它吃掉（Python 的 comp_for 也只允许到 or_test 层级，
        # 同一个道理——这是我实现时撞上来的坑）。
        if allow_ternary and min_prec == 0 and self._at_kw("如果"):
            return self._desugar_ternary(left)
        return left

    def _desugar_ternary(self, then_expr: A.Node) -> A.Node:
        """`甲 如果 条件 否则 乙` —— **解析期脱糖**（M36），不新增 AST 节点。
        ...
        为什么不做成新的表达式节点：那要在三个执行器 + 两个宿主各写一遍求值
        逻辑（还可能碰字节码格式）。脱糖之后走的是**既有**的函数调用与 if
        语义，三执行器与两宿主零改动、行为必然一致——本项目第六次用这个手法
        （前五次：`超()`、集合字面量、`用 … 为`、模式匹配、枚举）。

        语义要点：

        - **只求值一次**：`甲`/`乙` 只有被选中的那个会求值（`返回` 之后的语句
          不执行），与 Python 一致；把待选值塞进临时变量那种写法反而会两个都算。
        - **右结合**：`a 如果 c1 否则 b 如果 c2 否则 d` 解析成
          `a 如果 c1 否则 (b 如果 c2 否则 d)`——「否则」后面仍按完整表达式解析。
        - **条件位置**用完整的表达式层级，所以 `甲 如果 非 x 否则 乙`、
          `甲 如果 x 在 列表 否则 乙` 都成立。
        """
        kw = self._next()                     # 消费「如果」
        cond = self._parse_expr()
        if not self._at_kw("否则"):
            self._err(ParseUnexpectedError,
                      "条件表达式少了「否则」",
                      tok=self._peek(),
                      hint="写法是「甲 如果 条件 否则 乙」——"
                           "例如：令 等级 = \"成年\" 如果 年龄 >= 18 否则 \"未成年\"")
        self._next()                          # 消费「否则」
        other = self._parse_expr()            # 右结合：这里还会再嵌套
        line, col = kw.line, kw.col
        body = [A.If(
            branches=[(cond, [A.Return(value=then_expr, line=then_expr.line,
                                       col=then_expr.col)])],
            orelse=[A.Return(value=other, line=other.line, col=other.col)],
            line=line, col=col)]
        lam = A.Lambda(params=[], body=body, defaults=[], annotations=[],
                       param_kind=[], line=line, col=col)
        return A.Call(func=lam, args=[], keywords=[], line=line, col=col)
...
def parse_expression(text: str, filename: str = "<调试>") -> A.Node:
    """把一段文本解析成**单个表达式**的 AST 节点（M37，调试器「查看」用）。

    与 `parse()` 的分工：`parse()` 要一整段合法程序；调试器里写的是
    「查看 单价 * 数量」这种**随手一写**的片段，不该要求它是个程序，
    所以单独给一个入口，而不是让调用方去碰 `Parser._parse_expr()` 这类内部方法
    （那种「调试器直接伸手进解析器私有方法」的写法，改解析器时就容易漏掉它）。

    多余的内容（`查看 x y`）**明确报错**：静默只看前半截会让人以为整句都算过了。
    """
    source = text.replace("\r\n", "\n").replace("\r", "\n")
    tokens = tokenize(source, filename)
    p = Parser(tokens, source.split("\n"), filename)
    node = p._parse_expr()
    # 词法器会在末尾补一个 NEWLINE（行结束）再补 EOF，所以先吃掉那个 NEWLINE。
    # （M37 实测：漏了这一步，`查看 单价 * 数量` 会报「只能写一个表达式」——
    #  明明写的就是一个表达式，用户只会觉得调试器坏了。）
    if p._peek().type == "NEWLINE":
        p._next()
    if p._peek().type != "EOF":
        p._err(ParseUnexpectedError,
               "「查看」后面只能写一个表达式",
               hint="例如：查看 单价 * 数量 —— 想看多句话请改脚本后重新跑一次")
    return node
