...
| 4   | INT     | `v.i` 为 64 位整数                                     |
| 5   | FLOAT   | `v.d` 为双精度浮点                                     |
| 6   | HOST    | `v.u` 为宿主句柄号（列表/字典/模块/Python 对象）        |
| 7   | CELL    | `v.u` 为闭包单元号                                     |
| 8   | FUNC    | `v.u` 为基石函数对象号                                 |
| 9   | ITER    | `v.u` 为 C 侧迭代器 `JsIterC*`                          |
| 10  | STR     | `v.u` 为 C 侧原生字符串 `JsStr*`（M47 阶段二①）         |

**原生字符串（tag 10，M47 阶段二①）**：不可变的 UTF-8 字符串 + 引用计数，
字节与头部一块 `malloc`（`{int32 refcnt; int32 blen}` 紧跟字节，末尾补 `\0`）。
存在的理由：文本若只以 HOST 句柄表示，C 侧就看不见内容，于是每次 `甲 + "x"`
都要一次宿主回调（实测固定约 2.4µs），永远追不上树遍历直接调 Python 的 C 级
`str.__add__`（约 0.05µs）。**字符串不可变**，所以没有容器那种「别名 + 逃逸」
的问题（那正是「C 侧原生列表/字典」走不通的原因，见 §9）。

- 拼接在 `binop_fast`（`+`，两边都是 STR）；比较在 `compare_fast`
  （`== != < > <= >=`，`memcmp`——**UTF-8 的字节序就是码点序**，所以与 Python
  的按码点比较逐位一致）。其余运算（`*` 重复、`%` 格式化、`是/不是/在/不在`）
  照旧回落宿主。
- 宿主侧入口三个公开函数：`jsvm_str_new` / `jsvm_str_len` / `jsvm_str_bytes`；
  批量用 `jsvm_str_new_batch`（文本迭代器一次建整批）。
  语言绑定（如 Python）为省热路径上的 FFI，会**直接镜像** `JsStr` 头部，
  因此那份布局是**跨语言契约**——改 C 侧必须同步改绑定侧。
...
param_kind   形参种类（M25）：0=普通 1=*参数 2=**选项
instrs       指令流
...
扁平化后 `code_params` **每个形参占 4 个 int32**：
`param_idx, param_local, param_cell, param_kind`。
（M25 之前是 3 个；加第 4 个是为了让 C 侧 `bind_args` 知道谁负责收集多余的
实参——`*参数` 收多余的位置实参、`**选项` 收未匹配的关键字。）
...
2. `*参数` 收集多余的位置实参成列表、`**选项` 收集未匹配的关键字成字典（M25）；
   两者缺席时绑定空容器（Python 语义）；
3. 参数过多（且无 `*参数`）/ 缺少 / 未知关键字（且无 `**选项`）→ 回调宿主抛中文错误；
4. 压入新帧，`base = 原 sp - (1 + 位置参数数 + 关键字数)`；
5. `RETURN`：把返回值留在 `base`，清掉其上的一切，弹帧。
...
⚠️ **缓冲的所有权不变式（M47 修正）**：`it->buf[0..nbuf-1]` **每个槽位都由
缓冲区持有一份引用**，消费时**额外** retain 一次给栈。因此「**换批前**」与
「**销毁时**」都必须**整段**释放（`release_buf()`）。
早先「换批不释放 + 销毁只从 `it->pos` 起放」等于假定「消费即转移所有权」，
与消费处的 retain 配不上 —— **每消费一个元素漏一份引用**（实测遍历 1000 个
文本一次漏约 33KB），而且**不报错、对拍也全绿**，只在长跑进程里体现为
内存一直涨。同一不变式也约束宿主：`iter_next_batch` 填的每个 `out[i]`
必须是宿主新持有的一份引用，**不许**重复填同一个值的引用。
