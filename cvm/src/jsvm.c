 ...
 *   「循环 n 次」计数循环、闭包单元读写、
 *   **原生字符串的拼接与比较（M47 阶段二①，见下面的 JsStr）**。
 */
...
/* 不可变 UTF-8 字符串 + 引用计数。字节数据**紧跟头部**（一次 malloc），
 * 所以 `(char *)s + sizeof(JsStr)` 就是首字节。
 *
 * ⚠️ 这个布局是**跨语言契约**：jishi/cvm_bind.py 的 `_JsStr` 按同一布局
 * 直读头部（为了省掉每次转换的 FFI 往返）。改这里必须同步改那边，
 * tests/test_cvm.py 有一次性往返测试钉住它。
 *
 * 为什么不复用 HOST 句柄：句柄背后是 Python 对象，C 侧看不见内容，于是
 * 每次拼接都要一次 FFI 往返（固定 ~2.4µs），而树遍历直接调 Python 的
 * C 级 str.__add__（~0.05µs）—— 差 48 倍，永远追不上。
 * 字符串**不可变**，所以没有容器那种「别名 + 逃逸」的坑（M46 ③④ 就是被
 * 那个坑逼着换了方案），这是它与列表/字典最关键的区别。 */
typedef struct JsStr {
    int32_t refcnt;
    int32_t blen;      /* UTF-8 字节数（码点数 C 侧暂时用不到，故不缓存） */
} JsStr;

static char *str_bytes(JsStr *s) { return (char *)s + sizeof(JsStr); }

/* 分配一块 n 字节的原生字符串（内容未初始化），refcnt = 1。 */
static JsStr *str_alloc(int32_t n) {
    JsStr *s = (JsStr *)malloc(sizeof(JsStr) + (size_t)n + 1);
    if (!s) return NULL;
    s->refcnt = 1;
    s->blen = n;
    str_bytes(s)[n] = '\0';   /* 结尾补 0：方便调试，不参与语义 */
    return s;
}

static JVal make_str(JsStr *s) {
    JVal x; memset(&x, 0, sizeof x); x.tag = JS_TAG_STR;
    x.v.u = (uint64_t)(uintptr_t)s; return x;
}

static JsStr *as_str(JVal v) { return (JsStr *)(uintptr_t)v.v.u; }

static void str_release(JsStr *s) { if (s && --s->refcnt <= 0) free(s); }

/* 逐字节比较。UTF-8 的性质：字节序 == 码点序，所以 `memcmp` 的结果与
 * Python（按码点比较）完全一致——包括中文、emoji 与代理项。 */
static int str_cmp(JsStr *a, JsStr *b) {
    int32_t n = a->blen < b->blen ? a->blen : b->blen;
    int c = n ? memcmp(str_bytes(a), str_bytes(b), (size_t)n) : 0;
    if (c) return c < 0 ? -1 : 1;
...
typedef struct {
    ...
    JVal ret_val;          /* 挂起的返回值 */
    int call_line;         /* 本帧**被调进来**的位置（1 基；主帧为 0）——
                              报错的「调用链」与调试器都要它（M40） */
    int call_col;
} JsFrame;

/* 待冲刷的「追加」批次（M46 阶段一③）：见 JsVm.pa 的注释。 */
typedef struct JsAppendBatch {
    uint32_t handle;   /* 目标列表的宿主句柄 */
    JVal holder;       /* 批次持有的对象引用（落盘前句柄不能死） */
    JVal *vals;        /* 攒着的追加值（按序；所有权归本批次） */
    int n, cap;
} JsAppendBatch;

/* 待冲刷的「字典写入」批次（M46 阶段一④）：见 JsVm.pd 的注释。 */
typedef struct JsDictBatch {
    uint32_t handle;   /* 目标字典的宿主句柄 */
    JVal holder;
    JVal *kvs;         /* 每项 2 个：键、值（按序） */
    int32_t *lc;       /* 每项 2 个：行、列——落盘时要按项报错，行号不能糊 */
    int n, cap;
} JsDictBatch;

/* 句柄「种类」标记（M46 ③④）：只有宿主在 build_list / build_dict 里造出来的
 * 容器才置位。**只对确定是纯 list / 纯 dict 的目标做「延迟合并写」**——
 * 那种目标的落盘操作不会失败（list.extend）/ 失败也能按项报准行号，
 * 而别的目标照旧同步回调，语义一模一样。 */
enum { HK_UNKNOWN = 0, HK_LIST = 1, HK_DICT = 2 };
...
typedef struct JsVm {
    ...
    /* 语句起始表（M40）：与 instrs 等长，非 0 表示该指令是某语句的第一条。
     * 调试器靠它认出「语句边界」；NULL = 没设（不调试）。 */
    int32_t *stmt_marks; int nstmt_marks;
    /* 语句钩子（M40）：每条语句首指令执行前回调；NULL = 不回调。 */
    JsStmtHook stmt_hook; void *stmt_hook_ud;
    uint32_t *hrefcnt; int hcap;   /* HOST 句柄引用计数 */
    /* 每个句柄的「种类」（M46 ③④）：见 HK_* 枚举。与 hrefcnt 同步扩容 / 释放。 */
    uint8_t *hkind;
    JVal last_value;
    ...
    int stop_depth;
    /* 待冲刷的「追加」批次（M46 阶段一③）——**延迟合并写**。
     * 每个表项 = 一个 HOST 句柄 + 它攒着还没落盘的追加值（按序）。
     * 目的：把「循环里追加 N 次」的 N 次宿主回调压成 1 次——实测每次宿主
     * 回调固定约 2.4µs（C↔Python 的 FFI 往返），是 C VM 最大的固定开销。
     * ⚠️ **不变式：任何宿主回调之前必须 flush_pending()**，否则宿主会看到
     * 一个「少了最后几条」的列表（静默算错）。见 HC() 宏：全部宿主调用点
     * 都套了它，就是为这条。 */
    JsAppendBatch *pa; int pa_n, pa_cap;
    /* 待冲刷的「字典写入」批次（M46 阶段一④）：与 pa 同一套思路，
     * 把「循环里 字[键] = 值 N 次」压成 1 次。同上，靠 HC() 宏保证冲刷。 */
    JsDictBatch *pd; int pd_n, pd_cap;
} JsVm;
...
static int ensure_hcap(JsVm *vm, uint32_t id) {
    ...
    vm->hrefcnt = p;
    /* M46 ③④：hkind 与 hrefcnt 同步扩容（两者同生命周期） */
    uint8_t *q = (uint8_t *)realloc(vm->hkind, (size_t)ncap);
    if (!q) return -1;
    memset(q + vm->hcap, 0, (size_t)(ncap - vm->hcap));
    vm->hkind = q;
    vm->hcap = ncap;
    return 0;
}
...
/* -- 原生字符串（M47 阶段二①）：宿主侧入口，见 jsvm.h 的说明 ---------- */

JSVM_API int jsvm_str_new(JsVm *vm, const char *utf8, int32_t blen, JVal *out) {
    (void)vm;                     /* 预留：将来做常量驻留（interning）用 */
    if (blen < 0) return -1;
    if (blen > 0 && !utf8) return -1;
    JsStr *s = str_alloc(blen);
    if (!s) return -1;
    if (blen > 0) memcpy(str_bytes(s), utf8, (size_t)blen);
    *out = make_str(s);
    return 0;
}

JSVM_API int32_t jsvm_str_len(const JVal *v) {
    if (!v || v->tag != JS_TAG_STR) return -1;
    return as_str(*v)->blen;
}

JSVM_API const char *jsvm_str_bytes(const JVal *v) {
    if (!v || v->tag != JS_TAG_STR) return NULL;
    return str_bytes(as_str(*v));
}

JSVM_API int jsvm_str_new_batch(JsVm *vm, const char *buf,
                                const int32_t *offsets, int n, JVal *out) {
    (void)vm;
    if (n < 0 || (n > 0 && (!buf || !offsets || !out))) return -1;
    for (int i = 0; i < n; i++) {
        int32_t a = offsets[i], b = offsets[i + 1];
        JsStr *s = (a >= 0 && b >= a) ? str_alloc(b - a) : NULL;
        if (!s) {
            /* 半途失败：把这一批已建的**全部**释放，不留半成品给调用方 */
            for (int k = 0; k < i; k++) str_release(as_str(out[k]));
            return -1;
        }
        if (b > a) memcpy(str_bytes(s), buf + a, (size_t)(b - a));
        out[i] = make_str(s);
    }
    return 0;
}
...
static void retain_val(JsVm *vm, JVal v) {
    ...
    case JS_TAG_STR:   as_str(v)->refcnt++; break;   /* M47 阶段二① */
    default: break;   /* 数值/UNSET 无需计数 */
    }
}

/* 把迭代器预取缓冲里的**全部**槽位放掉并清空。
 *
 * ⚠️ 不变式（M47 修正的漏引用 bug）：`it->buf[0 .. nbuf-1]` 每一个都由缓冲区
 * **持有引用**，消费时额外 retain 一次给栈。所以「换批前」与「销毁时」都必须
 * 整段释放——**不能**只从 `it->pos` 开始放（那等于假定「消费即转移所有权」，
 * 与消费处的 retain 配不上，每消费一个元素漏一份引用）。
 *
 * 症状很不显眼：不报错、对拍全绿，只是长跑进程（LSP / MCP / REPL）内存一直涨
 * ——遍历 1000 个文本一次就漏约 33KB。由 `.tmp-probe/leak_probe.py` 量出来。 */
static void release_buf(JsVm *vm, JsIterC *it) {
    for (int i = 0; i < it->nbuf; i++)
...
static void release_val(JsVm *vm, JVal v) {
    ...
    case JS_TAG_ITER: {
        ...
        /* 缓冲里的**每一个**槽位都由它自己持有引用，这里必须整段放掉（M47 修正，
         * 见 FOR_ITER 处的不变式说明）。原先只从 `it->pos` 开始放，等于假定
         * 「消费即转移所有权」，与消费处的 retain 配不上——每消费一个元素漏一份。 */
        release_buf(vm, it);
        vm->host->release_handle(it->host_id);
        free(it);
        break;
    }
    case JS_TAG_STR:   str_release(as_str(v)); break;   /* M47 阶段二① */
    default: break;
    }
}
...
/* 把一条追加记进目标句柄的批次（**接管 obj 与 val 的所有权**）。
 * 返回 0 成功；-1 = 内存不足（调用方原地回退成同步落盘，行为不变）。 */
static int pa_push(JsVm *vm, JVal obj, JVal val) {
    uint32_t h = (uint32_t)obj.v.u;
    JsAppendBatch *b = NULL;
    for (int i = 0; i < vm->pa_n; i++)
        if (vm->pa[i].handle == h) { b = &vm->pa[i]; break; }

    if (b == NULL) {
        /* 先把值数组备好、**最后才提交**（pa_n++）——失败要能原地退出，
         * 不然调用方没法干净回退（obj/val 的所有权还没真交出去）。 */
        if (vm->pa_n == vm->pa_cap) {
            int nc = vm->pa_cap ? vm->pa_cap * 2 : 4;
            JsAppendBatch *p = (JsAppendBatch *)realloc(
                vm->pa, (size_t)nc * sizeof *p);
            if (!p) return -1;
            vm->pa = p; vm->pa_cap = nc;
        }
        b = &vm->pa[vm->pa_n];
        b->handle = h; b->vals = NULL; b->n = 0; b->cap = 0;
    }
    if (b->n == b->cap) {
        int nc = b->cap ? b->cap * 2 : 16;
        JVal *p = (JVal *)realloc(b->vals, (size_t)nc * sizeof *p);
        if (!p) return -1;
        b->vals = p; b->cap = nc;
    }
    if (b->n == 0) {
        b->holder = obj;        /* 新批次：存下对象引用（句柄落盘前不能死） */
        b->vals[b->n++] = val;
        vm->pa_n++;             /* 提交 */
        return 0;
    }
    release_val(vm, obj);       /* 已有批次：这一份 obj 用不上 */
    b->vals[b->n++] = val;
    return 0;
}

/* 把一条「字[键] = 值」记进目标句柄的批次（**接管 obj/key/val 的所有权**）。
 * line/col **按项存**：字典写入可能失败（键不可哈希），落盘时要按项报准行号。
 * 返回 0 成功；-1 = 内存不足（调用方原地回退成同步落盘，行为不变）。 */
static int pd_push(JsVm *vm, JVal obj, JVal key, JVal val,
                   int line, int col) {
    uint32_t h = (uint32_t)obj.v.u;
    JsDictBatch *b = NULL;
    for (int i = 0; i < vm->pd_n; i++)
        if (vm->pd[i].handle == h) { b = &vm->pd[i]; break; }

    if (b == NULL) {
        if (vm->pd_n == vm->pd_cap) {
            int nc = vm->pd_cap ? vm->pd_cap * 2 : 4;
            JsDictBatch *p = (JsDictBatch *)realloc(
                vm->pd, (size_t)nc * sizeof *p);
            if (!p) return -1;
            vm->pd = p; vm->pd_cap = nc;
        }
        b = &vm->pd[vm->pd_n];
        b->handle = h; b->kvs = NULL; b->lc = NULL; b->n = 0; b->cap = 0;
    }
    if (b->n == b->cap) {
        int nc = b->cap ? b->cap * 2 : 8;
        JVal *pk = (JVal *)realloc(b->kvs, (size_t)nc * 2 * sizeof *pk);
        if (!pk) return -1;
        b->kvs = pk;
        int32_t *pl = (int32_t *)realloc(b->lc, (size_t)nc * 2 * sizeof *pl);
        if (!pl) return -1;
        b->lc = pl; b->cap = nc;
    }
    b->kvs[b->n * 2] = key;
    b->kvs[b->n * 2 + 1] = val;
    b->lc[b->n * 2] = (int32_t)line;
    b->lc[b->n * 2 + 1] = (int32_t)col;
    if (b->n == 0) {
        b->holder = obj;        /* 新批次：存下对象引用（落盘前句柄不能死） */
        b->n++;
        vm->pd_n++;             /* 提交 */
        return 0;
    }
...
/* 把攒下的追加与字典写入一次性交给宿主落盘。
 * ⚠️ **任何宿主回调之前都必须先跑它**（见 HC 宏）——否则宿主会看到一个
 * 「少了最后几条」的容器。
 * 做法上先把 pa/pd 整体摘下来：落盘时释放值会回调宿主（release_handle），
 * 重入进来看到计数为 0，不会递归。 */
static int flush_pending(JsVm *vm) {
    int rc = 0;
    if (vm->pa_n > 0) {
        JsAppendBatch *batches = vm->pa;
        int n = vm->pa_n;
        vm->pa = NULL; vm->pa_n = 0; vm->pa_cap = 0;
        for (int i = 0; i < n; i++) {
            JsAppendBatch *b = &batches[i];
            if (b->n > 0 && rc == 0)
                rc = vm->host->list_append_many(b->handle, b->vals, b->n);
            for (int k = 0; k < b->n; k++)
                release_val(vm, b->vals[k]);
            free(b->vals);
            release_val(vm, b->holder);   /* 放掉批次持有的那份对象引用 */
        }
        free(batches);
    }
    if (vm->pd_n > 0) {
        JsDictBatch *batches = vm->pd;
        int n = vm->pd_n;
        vm->pd = NULL; vm->pd_n = 0; vm->pd_cap = 0;
        for (int i = 0; i < n; i++) {
            JsDictBatch *b = &batches[i];
            if (b->n > 0 && rc == 0)
                rc = vm->host->dict_setitem_many(b->handle, b->kvs, b->lc,
                                                 b->n);
            for (int k = 0; k < b->n * 2; k++)
                release_val(vm, b->kvs[k]);
            free(b->kvs); free(b->lc);
            release_val(vm, b->holder);
        }
        free(batches);
    }
    return rc;
...
/* ⚠️ **所有宿主回调调用点都必须经由本宏**（M46 阶段一③）。
 * 它保证「待冲刷的追加」一定在宿主看到列表**之前**落盘——
 * 漏掉任何一处，都会变成「列表少了最后几条」的静默算错。 */
#define HC(vm, m) (flush_pending(vm), (vm)->host->m)
...
JSVM_API void jsvm_destroy(JsVm *vm) {
    if (!vm) return;
    /* M46 阶段一③：兜底把攒下的追加落盘（正常路径 jsvm_run 收尾已经冲过；
       这里防的是「run 被中断 / 直接 destroy」的路径，避免泄漏批次内存）。 */
    flush_pending(vm);
    ...
    free(vm->hrefcnt);
    free(vm->hkind);        /* M46 ③④：与 hrefcnt 同生命周期 */
    free(vm);
}
...
/* 把当前帧栈（除主帧）拍成扁平数组交给宿主，供报错印「调用链」（M40）。
 * 必须在**弹帧之前**调：调用点信息随帧一起消失。
 * flat 布局：每层 3 个 int32 —— code_idx / call_line / call_col。 */
static void note_frames(JsVm *vm) {
    if (!vm->host || !HC(vm, note_frames)) return;
    int depth = vm->nframes - 1 - vm->stop_depth;
    if (depth < 0) depth = 0;
    if (depth > JS_MAX_FRAMES) depth = JS_MAX_FRAMES;
    int32_t flat[JS_MAX_FRAMES * 3];
    int n = 0;
    for (int i = vm->stop_depth + 1; i < vm->nframes; i++) {
        JsFrame *f = &vm->frames[i];
        flat[n++] = f->code_idx;
        flat[n++] = f->call_line;
        flat[n++] = f->call_col;
    }
    /* depth 为 0 也要通知：宿主拿它当「这次没有调用链」，否则会留着上一次的旧值 */
    HC(vm, note_frames)(flat, depth);
}
...
static int bind_args(JsVm *vm, int code_idx,
    ...
    if (nargs > npos) {
        ...
        JVal lst;
        if (HC(vm, build_list)(args + npos, nargs - npos, &lst,
                                 line, col) != 0)
            goto fail;
    ...
    /* 3) 关键字实参：先匹配普通参数名，剩下的进 **选项 */
    {
        ...
        for (int k = 0; k < nkw; k++) {
            ...
            if (found >= 0) {
                ...
                free(extra_names); free(extra_vals);
                HC(vm, raise_bad_kw)(code_idx, name, line, col);
                goto fail;
            }
        }
        ...
        if (dstar_pos >= 0) {
            JVal dct;
            if (HC(vm, pack_kwargs)(extra_names, extra_vals, nextra,
                                      &dct, line, col) != 0) {
                free(extra_names); free(extra_vals);
                goto fail;
            }
    ...
    if (star_pos >= 0 && bound[star_pos].tag == JS_TAG_UNSET) {
        JVal lst;
        if (HC(vm, build_list)(args, 0, &lst, line, col) != 0) goto fail;
        bound[star_pos] = lst;
    }
    if (dstar_pos >= 0 && bound[dstar_pos].tag == JS_TAG_UNSET) {
        JVal dct;
        if (HC(vm, pack_kwargs)(NULL, NULL, 0, &dct, line, col) != 0)
            goto fail;
        bound[dstar_pos] = dct;
    }
    ...
    /* 6) 缺参检查（只看普通参数；*参数 / **选项 已保证有值） */
    {
        ...
        if (nmiss > 0) {
            HC(vm, raise_missing)(code_idx, miss, nmiss, line, col);
            goto fail;
        }
    }
...
fail_argc:
    HC(vm, raise_argc)(code_idx, nargs + nkw, line, col);
    /* fall through */
fail:
...
/* 取**当前指令之后第 k 条**指令的字段 f（M48 方法调用融合要看紧跟的 CALL）。
 * 用 int64 中间量：`ip + 1` 不会溢出，但下标算式显式写出来更不容易看错。 */
#define INSN_AT(k, f) (instrs[((size_t)(ip) + (size_t)(k)) * 6 + (f)])
...
static int exc_is_signal(JsVm *vm, JVal *exc, int *out) {
    return HC(vm, is_signal)(exc, out);
}
...
static int take_pending_err(JsVm *vm, JVal *out) {
    if (vm->pending_err.tag == JS_TAG_UNSET) {
        ...
        if (HC(vm, error_handle)(out) != 0) return -1;
 ...
 *
 * 信号约定（与宿主 `_cb_is_signal` 对齐）：
 *   1 = 中断 / 2 = 继续：循环与处理器谁更内层谁先接（与 vm.py 一致）；
 *   3 = **卸载类**（调试器的「退出」）：只穿 `最终` 块，**既不被 `捕获` 接住、
 *       也不被循环接住**——与 Python VM 的 `_UNWIND_SIGNALS` 同一语义（M40）。 */
static int dispatch_exception(JsVm *vm, JVal *exc) {
    int sig = 0;
    if (exc_is_signal(vm, exc, &sig) != 0) return 0;
    int unwind_only = (sig == 3);
    while (vm->nframes > vm->stop_depth) {
        ...
        if (sig) {
            /* 信号：循环与处理器谁更内层谁先接（与 vm.py 一致）。
               **卸载类信号（sig==3，调试器的「退出」）不走循环**：它只该穿
               `最终` 块（下面那个 h 分支），不该被 `中断`/`继续` 的循环接住。 */
            if (!unwind_only && lp && (!h || lp->seq > h->seq)) {
            ...
            if (h) {
                /* 交给处理器：
                   - 普通/循环信号 → 跳到 `catch_ip`（编译器生成的「检查信号 →
                     是信号就重新抛出」入口，所以**不会**误入用户的 `捕获` 体）；
                   - **卸载类信号（sig==3）→ 直接跳 `finally_ip`**（有的话），
                     跳过捕获检查：它只需要穿 `最终` 块，`捕获` 本来就不该接它。
                     跑完 `最终` 由 END_FINALLY/THROW 继续向外（M40）。 */
                int target = h->catch_ip;
                if (unwind_only && h->finally_ip >= 0) target = h->finally_ip;
                while (vm->sp > h->sp)
                    release_val(vm, vm->stack[--vm->sp]);
                retain_val(vm, *exc);
                PUSH(vm, *exc);
...
static int binop_fast(int op, JVal l, JVal r, JVal *out, JsVm *vm,
                      int line, int col) {
    /* 原生字符串拼接（M47 阶段二①）：两边都是原生字符串时 C 侧就地拼，
     * 不再回调宿主 —— 「循环里 甲 = 甲 + "x" 2000 次」因此从 2000 次
     * FFI 往返降到 0。`*`（重复）等其它运算回落宿主，语义不变。 */
    if (l.tag == JS_TAG_STR && r.tag == JS_TAG_STR) {
        if (op != JS_BIN_ADD) return 0;
        JsStr *ls = as_str(l), *rs = as_str(r);
        int64_t total = (int64_t)ls->blen + (int64_t)rs->blen;
        /* 超过 int32 表示长度：回落宿主（由 Python 去决定报什么错） */
        if (total > 0x7FFFFFFF) return 0;
        JsStr *s = str_alloc((int32_t)total);
        if (!s) return 0;                      /* 内存不足同样回落，语义不变 */
        memcpy(str_bytes(s), str_bytes(ls), (size_t)ls->blen);
        memcpy(str_bytes(s) + ls->blen, str_bytes(rs), (size_t)rs->blen);
        *out = make_str(s);
        return 1;
    }
...
static int compare_fast(int op, JVal l, JVal r, JVal *out) {
    /* 原生字符串比较（M47 阶段二①）：`memcmp` 走 UTF-8 字节序，而 UTF-8
     * 的字节序**就是**码点序，所以与 Python 的按码点比较逐位一致。
     * 「是/不是/在/不在」不走这里（它们有各自的语义），回落宿主。 */
    if (l.tag == JS_TAG_STR && r.tag == JS_TAG_STR) {
        int c = str_cmp(as_str(l), as_str(r));
        switch (op) {
        case JS_CMP_EQ: *out = make_bool(c == 0); return 1;
        case JS_CMP_NE: *out = make_bool(c != 0); return 1;
        case JS_CMP_LT: *out = make_bool(c <  0); return 1;
        case JS_CMP_GT: *out = make_bool(c >  0); return 1;
        case JS_CMP_LE: *out = make_bool(c <= 0); return 1;
        case JS_CMP_GE: *out = make_bool(c >= 0); return 1;
        default: return 0;
        }
    }
...
static int truthy_fast(JVal v, JsVm *vm, int *out) {
    ...
    default:
        return HC(vm, truthy)(&v, out, 0, 0) == 0 ? 1 : -1;
    }
}
...
#define SYNC_FRAME() do { SAVE_IP(); f = &vm->frames[vm->nframes-1]; \
        ...
        for (;;) {
            ...
            /* 语句钩子（M40）：只在**语句首指令**上回调一次。判断放在这里
             * （而不是每条指令都进一次回调）——不挂调试器时只多一次指针判空。 */
            if (vm->stmt_hook && vm->stmt_marks) {
                int gi = code->instr_off + ip;
                if (gi < vm->nstmt_marks) {
                    int32_t sl = vm->stmt_marks[gi];
                    if (sl) {
                        SAVE_IP();
                        if (vm->stmt_hook(vm->stmt_hook_ud, f->code_idx,
                                          (int)sl, vm->nframes) != 0) {
                            /* 钩子请求中止（调试器的「退出」）：宿主已把
                               `DebugQuit` 放进 pending_err，交给**异常分发**
                               走一遍——于是 `最终` 块照常执行、`捕获` 接不住它
                               （信号值 3 = 卸载类，见 dispatch_exception）。
                               M39 在 Python VM 上就是这么做的，两边语义一致。 */
                            JVal exc;
            ...
            case JS_OP_LOAD_FAST: {
                ...
                if (v.tag == JS_TAG_UNSET) {
                    SAVE_IP();
                    HC(vm, raise_unbound_local)(f->code_idx, a, line, col);
                    goto dispatch_err;
                }
                retain_val(vm, v);
                PUSH(vm, v);
            ...
            case JS_OP_LOAD_DEREF: {
                ...
                if (cell->v.tag == JS_TAG_UNSET) {
                    SAVE_IP();
                    HC(vm, raise_unbound_cell)(f->code_idx, a, line, col);
                    goto dispatch_err;
                }
            ...
            case JS_OP_BIN_OP: {
                ...
                if (fast) {
                    ...
                    SAVE_IP();
                    if (HC(vm, binop)(a, &l, &r, &out, line, col) != 0) {
                        release_val(vm, l); release_val(vm, r);
                        goto dispatch_err;
                    }
                    release_val(vm, l); release_val(vm, r);
            ...
            case JS_OP_UNARY_OP: {
                ...
                if (done) {
                    ...
                    SAVE_IP();
                    if (HC(vm, unary)(a, &v, &out, line, col) != 0) {
            ...
            case JS_OP_COMPARE: {
                ...
                if (compare_fast(a, l, r, &out)) {
                    ...
                    SAVE_IP();
                    if (HC(vm, compare)(a, &l, &r, &out, line, col) != 0) {
                        release_val(vm, l); release_val(vm, r);
                        goto dispatch_err;
                    }
                    release_val(vm, l); release_val(vm, r);
            ...
            case JS_OP_CALL: {
                ...
                if (fn.tag == JS_TAG_FUNC) {
                    ...
                    if (!args || !kwv) { free(args); free(kwv);
                        HC(vm, raise_internal)("内存不足"); return -1; }
                    ...
                    if (vm->nframes >= JS_MAX_FRAMES) {
                        SAVE_IP();
                        HC(vm, raise_recursion)(line, col);
                    ...
                    for (int i = 0; i < ncellvars; i++) {
                        ...
                        if (!cells[i]) { HC(vm, raise_internal)("内存不足"); return -1; }
            ...
            case JS_OP_BUILD_CLASS: {
                ...
                JVal out;
                SAVE_IP();
                if (HC(vm, make_class)(&name, &base, methods, nmethods,
                                         &out, line, col) != 0) {
                    release_val(vm, name);
                    goto dispatch_err;
                }
            ...
            case JS_OP_MAKE_FUNCTION: {
                ...
                if (!jfn) { HC(vm, raise_internal)("内存不足"); return -1; }
                jfn->code_idx = a;
                jfn->ncells = ncells;
            ...
            case JS_OP_UNPACK: {
                ...
                if (!items) { release_val(vm, v);
                    HC(vm, raise_internal)("内存不足"); return -1; }
                SAVE_IP();
                if (HC(vm, unpack)(&v, n, star, items, line, col) != 0) {
            ...
            case JS_OP_LIST_APPEND: {
                ...
                JVal obj = POP(vm);
                SAVE_IP();
                /* M46 阶段一③「延迟合并写」：先攒在 C 侧，等下一个可能
                 * 「观察」到列表的宿主回调之前一次性落盘。
                 * 于是「循环里追加 5000 次」从 5000 次 FFI 往返压成 1 次。
                 * **只对 build_list 造出来的列表攒**（`hkind == HK_LIST`）——那种目标
                 * 必然是 list，落盘用 extend 不会失败，也就不会有「报错该算
                 * 哪一条」的歧义；别的目标照旧同步回调，错误语义不变。
                 * 攒不下（内存不足）同样原地退回同步路径。 */
                if (obj.tag == JS_TAG_HOST
                        && (int)obj.v.u < vm->hcap
                        && vm->hkind[(uint32_t)obj.v.u] == HK_LIST
                        && pa_push(vm, obj, val) == 0) {
                    PUSH(vm, make_null());  /* 追加返回空，保持 CALL 栈语义 */
                    break;
                }
                if (HC(vm, list_append)(&obj, &val, line, col) != 0) {
            ...
            case JS_OP_BUILD_SLICE: {
                ...
                if (a & 1) start = POP(vm);
                SAVE_IP();
                if (HC(vm, make_slice)(&start, &stop, &step, &out,
                                         line, col) != 0) {
            ...
            case JS_OP_BUILD_LIST: {
                JVal out;
                SAVE_IP();
                if (HC(vm, build_list)(&vm->stack[vm->sp - a], a,
                                         &out, line, col) != 0)
                    goto dispatch_err;
                vm->sp -= a;
                for (int i = 0; i < a; i++)
                ...
                PUSH(vm, out);
                /* M46 阶段一③：记下「这个句柄是纯 list」——后面 `X.追加(v)`
                 * 才敢走「延迟合并写」（见 hkind 字段的注释）。 */
                if (out.tag == JS_TAG_HOST
                        && (int)out.v.u < vm->hcap)
                    vm->hkind[(uint32_t)out.v.u] = HK_LIST;
                break;
            }
            ...
            case JS_OP_BUILD_DICT: {
                JVal out;
                SAVE_IP();
                if (HC(vm, build_dict)(&vm->stack[vm->sp - 2 * a], a,
                                         &out, line, col) != 0)
                    goto dispatch_err;
                int n = 2 * a;
                vm->sp -= n;
                for (int i = 0; i < n; i++)
                    release_val(vm, vm->stack[vm->sp + i]);
                PUSH(vm, out);
                /* M46 阶段一④：记下「这个句柄是纯 dict」——后面
                 * `字[键] = 值` 才敢走「延迟合并写」（见 hkind 的注释）。 */
                if (out.tag == JS_TAG_HOST
                        && (int)out.v.u < vm->hcap)
                    vm->hkind[(uint32_t)out.v.u] = HK_DICT;
                break;
            }
            ...
            case JS_OP_GET_ATTR: {
                JVal obj = POP(vm), out;
                SAVE_IP();
                if (HC(vm, getattr)(&obj, a, &out, line, col) != 0) {
            ...
            /* 方法调用融合（M48 阶段三②）：栈 [obj, 实参...] → 结果。
             *
             * `X.方法(实参)` 本来是 `GET_ATTR` + `CALL` 两条指令，即**两次**
             * 宿主回调。实测每次回调约 4µs（其中必要工作只有 2–3µs，其余是
             * ctypes 编组的固定开销），于是每次方法调用白花约 4µs。
             * 这条指令把两步合成一次回调。
             *
             * ⚠️ 试过两个更「省事」的做法，都行不通，记在这里免得重走：
             *   ① 在 `GET_ATTR` 里向前看那条 `CALL` 直接融合 —— 行不通：
             *      实参是在 GET_ATTR **之后**才逐条压栈的，一跳过就等于
             *      **实参永远没被求值**。
             *   ② 借 `GET_ATTR` 的 `b` 字段当「距离提示」—— 同样因为在
             *      GET_ATTR 处拿不到实参；而且 `1 + 实参数` 对**复合实参**
             *      （`甲.方法(乙.方法2(x))`）也不成立（内层调用占好几条指令）。
             * 所以只能发一条新指令，并且**只发给 C VM**（见编译器侧）。
             *
             * 错误语义与老路径**逐字一致**：老路径里「取属性失败」用 `GET_ATTR`
             * 的行列、「调用体里出错」用 `CALL` 的行列，两者**行相同、列不同**
             * （`甲.方法(...)`：属性名在第 2 列、左括号在第 5 列左右）。
             * 所以编译器把**属性名的列**塞在 `c` 里（属性名所在的位置就是老路径
             * `GET_ATTR` 的位置），这里据此分两次上报：取属性失败用 (line, c)、
             * 调用失败用 (line, col)。 */
            case JS_OP_METHOD_CALL: {
                int nargs = a;
                int name_idx = b;
                int gcol = c;                 /* 属性名列（= 老 GET_ATTR 的列） */
                int fn_base = vm->sp - (1 + nargs);
                JVal obj = vm->stack[fn_base];
                JVal out;
                SAVE_IP();
                if (HC(vm, method_call)(&obj, name_idx, &vm->stack[fn_base + 1],
                                        nargs, &out, line, gcol,
                                        line, col) != 0)
                    goto dispatch_err;
                while (vm->sp > fn_base)
            ...
            case JS_OP_SET_ATTR: {
                ...
                SAVE_IP();
                if (HC(vm, setattr)(&obj, a, &val, line, col) != 0) {
            ...
            case JS_OP_GET_ITEM: {
                ...
                SAVE_IP();
                if (HC(vm, getitem)(&obj, &idx, &out, line, col) != 0) {
            ...
            case JS_OP_SET_ITEM: {
                ...
                SAVE_IP();
                /* M46 阶段一④「字典写入延迟合并写」：与 ③ 的追加同一套思路，
                 * 把「循环里 字[键] = 值 N 次」压成 1 次宿主回调。
                 * **只对 build_dict 造出来的字典攒**——别的目标（自定义对象、
                 * 列表下标赋值…）照旧同步回调，语义与报错行号一模一样。 */
                if (obj.tag == JS_TAG_HOST
                        && (int)obj.v.u < vm->hcap
                        && vm->hkind[(uint32_t)obj.v.u] == HK_DICT
                        && pd_push(vm, obj, idx, val, line, col) == 0)
                    break;
                if (HC(vm, setitem)(&obj, &idx, &val, line, col) != 0) {
                    release_val(vm, obj); release_val(vm, idx);
            ...
            case JS_OP_GET_ITER: {
                ...
                int batchable = 0;
                SAVE_IP();
                if (HC(vm, iter_new)(&obj, &out, &batchable,
                                       line, col) != 0) {
                    release_val(vm, obj); goto dispatch_err;
                }
                release_val(vm, obj);
                ...
                if (!it) {
                    HC(vm, release_handle)((uint32_t)out.v.u);
                    HC(vm, raise_internal)("内存不足"); return -1;
                }
            ...
            case JS_OP_FOR_ITER: {
                ...
                if (itv.tag == JS_TAG_ITER) {
                    ...
                    /* ⚠️ 缓冲的所有权不变式（M47 修正）：
                     * `it->buf[0 .. nbuf-1]` **每一个都由缓冲区持有引用**，
                     * 消费时**额外** retain 一次给栈。所以
                     *   换批前 → 必须把上一批整段释放（见下面的 release_buf）
                     *   销毁时 → 必须把**整段**释放（见 release_val 的 ITER 分支）
                     * 早先销毁只从 `it->pos` 开始放，等于「消费即转移所有权」，
                     * 与消费处的 retain 配不上——**每消费一个元素漏一份引用**：
                     * 文本 32 字节/元素（一个 `JsStr` 块），列表/字典/函数同理。
                     * 遍历 1000 个文本一次就漏 33KB，LSP / MCP 这类长跑进程会一直涨。 */
                ...
                int done = 0;
                SAVE_IP();
                if (HC(vm, iter_next)(&itv, &out, &done, line, col) != 0)
                    goto dispatch_err;
                if (done) {
                    release_val(vm, POP(vm));
                    ip = a;
            ...
            case JS_OP_LOOP_SETUP: {
                ...
                if (n.tag == JS_TAG_INT) {
                    ...
                    SAVE_IP();
                    if (HC(vm, to_int)(&n, &out, line, col) != 0) {
            ...
            case JS_OP_IMPORT: {
                JVal out;
                SAVE_IP();
                if (HC(vm, do_import)(a, b, &out, line, col) != 0) goto dispatch_err;
                PUSH(vm, out);
                break;
            }
            ...
            case JS_OP_THROW: {
                ...
                if (HC(vm, make_error)(&v, line, col, &exc) != 0) {
                    release_val(vm, v);
                    goto dispatch_err;
                }
            ...
            case JS_OP_MATCH_EXC: {
                ...
                int matched = 0;
                if (HC(vm, match_error)(err, &cond, &matched,
                                          line, col) != 0) {
                    release_val(vm, cond);
                    goto dispatch_err;
                }
            ...
            case JS_OP_END_FINALLY: {
                ...
                /* 无挂起返回：检查有没有「待重新抛出的异常」。
                   正常路径走 finally 时 pending_err 是空的（异常根本没发生）；
                   而卸载类信号（`退出`）是**直接跳进 finally_ip** 的，它需要
                   在 finally 跑完后继续向外传播——否则信号就在这里被静静吞掉
                   （实测：`最终` 里的打印执行了，程序却「正常结束」了）。
                   与 Python 的 END_FINALLY「栈顶是异常就重新抛出」同一语义。 */
                if (vm->pending_err.tag != JS_TAG_UNSET) {
                    JVal exc = vm->pending_err;
                    if (dispatch_exception(vm, &exc)) {
                        /* 注意**不能**用 SYNC_FRAME：它会先 SAVE_IP()，把
                           `dispatch_exception` 刚设好的跳转目标冲掉
                           （此时 ip 还指着 END_FINALLY 的下一条）。 */
                        f = &vm->frames[vm->nframes - 1];
                        code = &vm->codes[f->code_idx];
                        instrs = vm->instrs +
                                 (size_t)code->instr_off * 6;
...
dispatch_err: {
            ...
            JVal exc;
            /* M46 阶段一③：报错前先把攒下的追加落盘——用户可能用
             * `尝试/捕获` 接住这个错，之后再去读那个列表；树遍历那边
             * 追加早就生效了，这里不补一次就是「静默少几条」。 */
            flush_pending(vm);
            /* 先拍「调用链」再取异常对象（M40）：此时帧栈还是完整的。
               取回异常对象会走宿主回调（error_handle），那边只关心异常本身；
               帧信息只能靠这一次快照带出去。 */
            note_frames(vm);
            if (take_pending_err(vm, &exc) != 0) return -1;
            if (dispatch_exception(vm, &exc)) {
...
JSVM_API int jsvm_frame_meta(JsVm *vm, int depth, int *out3) {
    if (depth < 0 || depth >= vm->nframes) return -1;
    JsFrame *f = &vm->frames[vm->nframes - 1 - depth];
    out3[0] = f->code_idx;
    out3[1] = f->call_line;
    out3[2] = f->call_col;
    return 0;
...
JSVM_API int jsvm_dump_locals(JsVm *vm, int depth) {
    if (depth < 0 || depth >= vm->nframes) return -1;
    if (!vm->host || !HC(vm, debug_local)) return 0;
    JsFrame *f = &vm->frames[vm->nframes - 1 - depth];
    JsCode *code = &vm->codes[f->code_idx];
    int ncells = code->ncellvars;
    /* 先局部槽、后单元：与 Code.local_names / cellvars 的排布一致，
       宿主按 (slot, is_cell) 就能还原出名字（它自己那侧有这两张表）。 */
    for (int i = 0; i < f->nlocals; i++) {
        if (f->locals[i].tag == JS_TAG_UNSET) continue;   /* 还没赋值的槽位不显示 */
        if (HC(vm, debug_local)(i, 0, -1, &f->locals[i]) != 0) return -1;
    }
    for (int i = 0; i < ncells; i++) {
        if (!f->cells[i]) continue;
        if (f->cells[i]->v.tag == JS_TAG_UNSET) continue;
        if (HC(vm, debug_local)(i, 1, -1, &f->cells[i]->v) != 0) return -1;
    }
    return 0;
}
...
JSVM_API int jsvm_dump_globals(JsVm *vm) {
    if (!vm->host || !HC(vm, debug_local)) return 0;
    for (int i = 0; i < vm->nnames; i++) {
        if (vm->globals[i].tag == JS_TAG_UNSET) continue;
        /* is_cell == 2：这是全局，slot 就是名字表下标 */
        if (HC(vm, debug_local)(i, 2, i, &vm->globals[i]) != 0) return -1;
    }
    return 0;
}
...
JSVM_API int jsvm_run(JsVm *vm) {
    ...
    if (push_frame(vm, vm->main_idx, 0, locals, main->nlocals, 0, 0) != 0) {
        free(locals);
    ...
    f->ncells = ncellvars;
    int rc = run_loop(vm);
    /* 收尾：把 C 侧攒下的追加落盘（M46 阶段一③）。程序正常跑完时，
     * 最后那批追加后面已经没有「宿主回调」去触发冲刷，必须在这里补一次。 */
    if (flush_pending(vm) != 0 && rc == 0) rc = -1;
    return rc;
}
...
JSVM_API int jsvm_call_function(JsVm *vm, JVal *fv,
    ...
    if (push_frame(vm, jfn->code_idx, base, locals, sub->nlocals,
    ...
    *out = vm->stack[base];
    ...
    /* M46 阶段一③：这是「回到宿主」的边界——宿主马上可能读那个列表，
     * 所以攒下的追加必须在这里落盘。 */
    flush_pending(vm);
    return 0;
...
fail:
    ...
    flush_pending(vm);        /* 同理：出错也可能被上层接住后继续读列表 */
    return -1;
}
