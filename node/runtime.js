 ...
 * 位运算混进逻辑值只会让人困惑 → 明确报错，与 Python 侧 `_as_bit_int` 一致。
 */
function asBitInt(v, func) {
  if (typeof v === 'boolean' || !isIntLike(v)) {
    throw typeErr(`「${func}」需要整数，但收到「${display(v)}」（${typeName(v)}）`);
  }
  return asBig(v);
}

/** 位移位数：非负整数，且不能大到把内存撑爆（上限与 Python 侧一致）。 */
function bitShiftCount(v, func) {
  if (typeof v === 'boolean' || !isIntLike(v)) {
    throw typeErr(`「${func}」需要整数，但收到「${display(v)}」（${typeName(v)}）`);
  }
  const n = toIndexNum(v);
  if (n < 0) throw typeErr(`「${func}」的位数不能是负数（收到 ${n}）`);
  if (n > 1000000) throw typeErr(`「${func}」的位数太大（收到 ${n}，上限 1000000）`);
  return BigInt(n);
}

/** 大整数结果回到安全范围就还原成 number（与 bigIntArith 同一约定）。 */
function bigToJs(r) {
  const MAX = BigInt(Number.MAX_SAFE_INTEGER);
  return (r >= -MAX && r <= MAX) ? Number(r) : r;
}

/** 内建函数的参数个数检查（`needArgs` 的措辞是给「方法」用的）。 */
function needBuiltinArgs(name, args, lo, hi) {
  if (args.length < lo || args.length > hi) {
    const need = lo === hi ? `${lo} 个` : `${lo} 到 ${hi} 个`;
    throw typeErr(`「${name}」需要 ${need}参数，但传了 ${args.length} 个`);
  }
}
  ...
  return {
    ...
    '整数': new Builtin('整数', (v) => {
      ...
      if (x === null || typeof x === 'boolean') {
        return x === null ? 0 : (x ? 1 : 0);
      }
      if (typeof x === 'object') {
        throw typeErr(`「${display(v)}」不能转成整数`);
      }
      const n = Math.trunc(Number(x));
      if (Number.isNaN(n)) throw valueErr(`「${display(v)}」不能转成整数`);
      return n;
    }),
    '小数': new Builtin('小数', (v) => {
      const x = unwrap(v);
      if (x === null) return new FloatBox(0);
      if (typeof x === 'boolean') return new FloatBox(x ? 1 : 0);
      if (typeof x === 'object') {
        throw typeErr(`「${display(v)}」不能转成小数`);
      }
      const n = Number(x);
      if (Number.isNaN(n)) throw valueErr(`「${display(v)}」不能转成小数`);
      return new FloatBox(n);
    }),
    '文本': new Builtin('文本', (v) => pyStr(v)),
    // 文本插值脱糖的目标（M40）：**内部保留名**。名字带 `$` 是因为 Python 侧
    // 词法器不认这个字符、用户写不出来，所以永远不会被 `导入 文本` 遮蔽。
    // 与 jishi/parser.py 的 INTERP_TO_TEXT 是同一个名字，语义同 `文本`。
    '$文本': new Builtin('$文本', (v) => pyStr(v)),
    ...
    // 内部用（M52）：命名以 `$` 开头，用户写不出来（词法器不认这个字符）。
    '$造模块': new Builtin('$造模块', (name, attrs) => makeModule(name, attrs)),
    '长度': new Builtin('长度', (v) => {
    ...
    // M41：`带下标` / `配对` 与 Python 的 enumerate / zip 同语义。
    // 立刻返回列表而不是惰性迭代器——与 Python 侧一致（见 runtime._b_enumerate）
    '带下标': new Builtin('带下标', (...args) => {
      if (args.length < 1 || args.length > 2) {
        throw typeErr('「带下标」需要 1 到 2 个参数，例如 带下标(列表)');
      }
      const seq = seqOf(args[0], true, undefined, undefined);
      if (seq === null) {
        throw typeErr(`「${display(args[0])}」不能当「带下标」的内容用`);
      }
      const start = args.length === 2 ? Number(args[1]) : 0;
      if (!Number.isInteger(start)) {
        throw typeErr(`「带下标」的起始值要是整数，得到了「${display(args[1])}」`);
      }
      return seq.map((v, i) => [start + i, v]);
    }),
    '配对': new Builtin('配对', (...args) => {
      if (args.length === 0) throw typeErr('「配对」至少要给一个可迭代对象');
      const seqs = args.map((a) => {
        const s = seqOf(a, true, undefined, undefined);
        if (s === null) {
          throw typeErr(`「${display(a)}」不能当「配对」的内容用`);
        }
        return s;
      });
      const n = Math.min(...seqs.map((s) => s.length));
      const out = [];
      for (let i = 0; i < n; i++) out.push(seqs.map((s) => s[i]));
      return out;
    }),
    ...
    // ⚠️ JS 的位运算符是 **32 位**（`2 ** 40 & 1` 会被截断），所以必须先转
    // BigInt 再算——否则与 Python 的任意精度行为不一致（M32 同类坑）。
    '位与': new Builtin('位与', (...args) => {
      needBuiltinArgs('位与', args, 2, 2);
      return bigToJs(asBitInt(args[0], '位与') & asBitInt(args[1], '位与'));
    }),
    '位或': new Builtin('位或', (...args) => {
      needBuiltinArgs('位或', args, 2, 2);
      return bigToJs(asBitInt(args[0], '位或') | asBitInt(args[1], '位或'));
    }),
    '位异或': new Builtin('位异或', (...args) => {
      needBuiltinArgs('位异或', args, 2, 2);
      return bigToJs(asBitInt(args[0], '位异或') ^ asBitInt(args[1], '位异或'));
    }),
    '位取反': new Builtin('位取反', (...args) => {
      needBuiltinArgs('位取反', args, 1, 1);
      return bigToJs(~asBitInt(args[0], '位取反'));
    }),
    '左移': new Builtin('左移', (...args) => {
      needBuiltinArgs('左移', args, 2, 2);
      return bigToJs(asBitInt(args[0], '左移') << bitShiftCount(args[1], '左移'));
    }),
    '右移': new Builtin('右移', (...args) => {
      needBuiltinArgs('右移', args, 2, 2);
      return bigToJs(asBitInt(args[0], '右移') >> bitShiftCount(args[1], '右移'));
    }),
...
class Module {
  constructor(name, attrs) {
    // 给标准库函数补上「模块.」前缀（M51）：命名参数的映射表按
    // 「模块.函数」索引 —— 全库有 19 组跨模块同名函数（`解析` 在
    // json / 参数 / 日期 都有），只按函数名查会歧义。前缀在这里统一加，
    // 省得在两百多处 `new R.Builtin(...)` 手写（手写就是「写三遍」+ 漂移源）。
    // 名字只影响 `toString()` 的显示，不影响调用。
    for (const k of Object.keys(attrs)) {
      const v = attrs[k];
      if (v instanceof Builtin) v.name = name + '.' + k;
    }
    this.name = name;
    this.attrs = attrs;
  }
...
// ---------------------------------------------------------------------------
// 命名参数（kwargs）→ 位置参数（M51）
// ---------------------------------------------------------------------------

/** 标准库函数签名表：`{模块: {函数: [必填个数, [参数名…]]}}`。
 *  由 `tools/gen_stdlib_sigs.py` 从 jishi/stdlib/*.py 生成 —— 参数叫什么名字
 *  只有一份事实源，宿主不手抄。 */
const SIGS = require('./stdlib_sigs');

/**
 * 把命名参数映射回位置参数。
 *
 * 以前宿主是**静默丢掉** kwargs 的：`加密.摘要("abc", 算法="md5")` 在里面变成
 * `摘要("abc")`，悄悄用默认的 sha256 —— 与 Python 侧结果不同，还一声不响。
 * 现在能安全映射的就映射，**不能安全映射的一律明确报错**（文案给出修法），
 * 绝不再静默。
 *
 * 不能安全映射的四种情形：
 *   - 该函数不在签名表里（宿主独有内建、纯 `*参数` 形式）
 *   - 参数名对不上
 *   - 同一个参数既给位置又给命名（给了两次）
 *   - **命名参数跳过了前面的可选参数** —— 宿主按位置调用，中间空一位
 *     就得补 `空` 顶上，而 `空` 未必等价于「不给」，所以宁可报错也不猜
 */
function mapKwargs(qualified, args, kwargs, line, col) {
  const keys = kwargs ? Object.keys(kwargs) : [];
  if (keys.length === 0) return args;

  const dot = qualified.lastIndexOf('.');
  const mod = dot < 0 ? '' : qualified.slice(0, dot);
  const fn = dot < 0 ? qualified : qualified.slice(dot + 1);
  const sig = mod && SIGS[mod] ? SIGS[mod][fn] : undefined;

  if (!sig) {
    throw typeErr(
      `宿主上「${qualified}」暂不支持命名参数（${keys.join('、')}）—— `
      + '请改用位置参数', line, col);
  }

  const required = sig[0];
  const params = sig[1];
  if (args.length > params.length) {
    throw typeErr(
      `函数「${qualified}」最多接受 ${params.length} 个参数，但传了 ${args.length} 个`,
      line, col);
  }
  const slots = new Array(params.length).fill(UNSET);
  for (let i = 0; i < args.length; i++) slots[i] = args[i];
  for (const k of keys) {
    const i = params.indexOf(k);
    if (i < 0) {
      throw typeErr(
        `函数「${qualified}」没有名为「${k}」的参数`
        + `（可用：${params.join('、') || '无'}）`, line, col);
    }
    if (slots[i] !== UNSET) {
      throw typeErr(`函数「${qualified}」的参数「${k}」被给了两次`, line, col);
    }
    slots[i] = kwargs[k];
  }
  for (let i = 0; i < required; i++) {
    if (slots[i] === UNSET) {
      throw typeErr(
        `函数「${qualified}」缺少必填参数「${params[i]}」`, line, col);
    }
  }
  let n = params.length;
  while (n > 0 && slots[n - 1] === UNSET) n--;   // 尾部的空缺截掉，让宿主默认值生效
  for (let i = 0; i < n; i++) {
    if (slots[i] === UNSET) {
      throw typeErr(
        `函数「${qualified}」的命名参数跳过了「${params[i]}」（它没有给值），`
        + `而宿主是按位置调用的、中间不能留空 —— 请把「${params[i]}」也补上，`
        + '或全部改用位置参数', line, col);
    }
  }
 ...
 *
 * 与 Python 侧 `_b_make_module` 同义：`导入 统计` 在编译期被展开成
 * 「调外壳函数、拿一个导出字典」，这一步把它包回**真正的模块** ——
 * 于是 `统计.求和(...)` 的语法一个字都不用改，`打印(统计)` 也还是
 * `<模块 统计>`（而不是一坨字典）。
 */
function makeModule(name, attrs) {
  // 基石的字典在 JS 里是 Map（见 buildDict）
  const table = (attrs instanceof Map) ? Object.fromEntries(attrs) : attrs;
  return new Module(String(name), table);
}
...
const DICT_METHODS = {
  ...
  // 返回**新字典**（同名键以参数为准，浅合并）——与就地改的「更新」分工清楚
  '合并': (obj, args, line, col) => {
    needArgs('合并', args, 1, 1, line, col);
    if (!(args[0] instanceof Map)) {
      throw typeErr(`「合并」需要一个字典参数，但传了「${display(args[0])}」（${typeName(args[0])}）`, line, col);
    }
    const merged = new Map(obj);
    for (const [k, v] of args[0]) merged.set(k, v);
    return merged;
  },
  '弹出': (obj, args, line, col) => {
